...
def initialize_embedding_model():
    ...
    if embedding_model is None:
        try:
            embedding_model = SentenceTransformer('sentence-transformers/msmarco-MiniLM-L-6-v3')
    ...
    if material_data_df is None:
        try:
            if os.path.exists(MATERIAL_CSV_PATH):
                ...
                logger.info(f"Loaded {len(material_data_df)} material records from {MATERIAL_CSV_PATH}")
            else:
                # Create an empty DataFrame with the required columns
                material_data_df = pd.DataFrame(columns=[
                    'material_name', 'element_type', 'material_type', 'volume', 
                    'area', 'layer_count', 'material_description', 'ec_per_kg', 'density'
                ])
    ...
    if material_embeddings is None:
        try:
            if os.path.exists(EMBEDDING_NPY_PATH):
                ...
                logger.info(f"Loaded {len(material_embeddings)} material embeddings from {EMBEDDING_NPY_PATH}")
            else:
...
def create_material_description(element_info):
    ...
    desc_parts = [
        f"ElementType:{element_info.get('element_type', 'Unknown')}"
    ]
    ...
    if element_info.get('material_type') == 'layered' and 'layer_materials' in element_info:
...
def add_material_to_database(element_data):
    ...
    try:
        ...
        # Add to DataFrame
        material_data_df = pd.concat([material_data_df, pd.DataFrame([record])], ignore_index=True)
...
def find_similar_material(element_data, min_similarity=0.7):
    ...
    if query_element_type:
        ...
        filtered_indices = material_data_df[material_data_df['element_type'] == query_element_type].index.tolist()
        
        if not filtered_indices:
            logger.warning(f"No materials found for element type '{query_element_type}', falling back to full database")
    ...
    try:
        ...
        if best_score >= min_similarity:
            ...
            logger.info(f"Found similar material '{best_match}' (score: {best_score:.3f}) with matching element type")
            return best_match, best_score
        else:
            logger.info(f"No similar material found above threshold {min_similarity} (best: {best_score:.3f})")
    ...
    if ifc_file:
        ...
        for type_name, elements in element_types.items():
            ...
            for element in elements:
                ...
                material_name = element_data.get('material_name')
                if material_name and material_name in MaterialList and material_name not in MaterialsToIgnore:
        ...
        logger.info(f"Found and added {materials_found} materials from IFC file to database")
...
def calculate_embodied_carbon(filepath):
    """Calculate the embodied carbon for an IFC model"""
    logger.info(f"Calculating embodied carbon for {filepath}")
    
    # Initialize the IFC file
    ifc_file = ifcopenshell.open(filepath)
    
    # Initialize material database with this IFC file
    logger.info("Initializing material database...")
    initialize_material_database(ifc_file)
    ...
    # Process slabs to be ignored (those in roofs)
    slabs_to_ignore = []
    if roofs:
        for roof in roofs:
            if not hasattr(roof, "IsDecomposedBy"):
                continue
    ...
    logger.info(f"Slabs to ignore (part of roofs): {slabs_to_ignore}")

    # Calculate embodied carbon for each element type
    total_ec = 0
