    ...
    for window in windows:
        ...
        if current_material_ec is None:
            if MATERIAL_REAPLCE:
                ...
                logger.warning(f"Material not found. Skipping this window.")
                continue
...
def calculate_piles(piles):
    ...
    for pile in piles:
        ...
        if dimensions is None:
            logger.error('Dimensions/Diameter not found')
        if dimensions:
            depthmm = dimensions.get('Depth')
            if depthmm is None:
                logger.error("Depth not found")
            else:
                depth = depthmm / 1000
        ...
        if rebar:
            rebar_no, area = rebar.split("H")
            rebar_vol = depth * int(rebar_no) * 3.14 * ((int(area)/2000) **2)
...
def calculate_footings(footings):
    ...
    for footing in footings:
        ...
        if rebar_set:
            ...
            logger.debug(f"Rebar_breadth: {rebar_breadth}")
            logger.debug(f"Rebar_height: {rebar_height}")
            logger.debug(f"Rebar_length: {rebar_length}")
            logger.debug(f"Perimeter: {perimeter}")
            logger.debug(f"Stirrups_vol: {stirrups_vol}")
                    ...
                    if property_def.is_a('IfcElementQuantity') and property_def.Name == 'Qto_FootingBaseQuantities':
                        for quantity in property_def.Quantities:
                            if quantity.is_a('IfcQuantityVolume') and (quantity.Name == 'NetVolume' or quantity.Name == 'GrossVolume'):
                                logger.debug(f'Found NetVolume  for {footing.Name}: {quantity.VolumeValue}')
                                quantities[quantity.Name] = quantity.VolumeValue
                                current_quantity = quantity.VolumeValue
                                break
                        if current_quantity is not None:
        ...
        if rebar_set == None:
            ...
            logger.debug(f"Vol of {footing.Name}'s rebars is {rebar_vol}")
            # print(rebar_vol)
            current_ec = material_ec_perkg * material_density * (current_quantity - rebar_vol)
            rebar_ec = rebar_vol  * 2.510 * 7850
...
if __name__ == "__main__":
    ...
    ifcpath = "/Users/jk/Downloads/z. Complex Models/Complex 3_USETHIS.ifc"
    ...
    print(f"Totaifcl Embodied Carbon: {total_ec:.2f} kgCO2e")
    if total_gfa > 0:
